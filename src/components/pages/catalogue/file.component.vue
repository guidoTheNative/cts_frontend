              ...
              <tbody>
                <tr v-for="(file, index) in files" :key="file.id" class="border-b">
                  ...
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{
                      file.subcategory == "undefined" ? "N/A" : file.subcategory
                    }}
                  </td>
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
              ...
              <div class="overflow-hidden sm:rounded-md">
                <div class="px-4 py-5 sm:p-6">
                  <div class="grid grid-cols-6 gap-6">
                    ...
                    <!-- Dropdown for Subcategory Items -->
                    <div class="col-span-6 sm:col-span-6">
                      <label for="subcategory" class="block text-sm font-medium text-gray-700">
                        Tag Image to Subcategory</label>
                      <div class="mt-1">
                        <select id="subcategory" v-model="selectedSubcategory" class="form-select"
                          @click="getsubcategoryItems">
                          <option value="" disabled>Select Subcategory</option>
                          <option v-for="item in subcategoryItems" :key="item.id" :value="item.name">{{ item.name }}
                          </option>
                        </select>
                      </div>
                    </div>
...
const { value: selectedSubcategory, errorMessage: pselectedSubcategoryError } =
  useField("selectedSubcategory");
...
const onSubmit = useSubmitForm((values, actions) => {
  let newValues = {
    ...
    subcategory: values.selectedSubcategory,
    metadata: {
      publicationDate: moment(model.value.publicationDate).format(moment.HTML5_FMT.DATE),
    },
  };
