    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div class="align-middle inline-block min-w-full">
        <ul class="nav nav-tabs flex flex-col md:flex-row flex-wrap pl-0 mb-4 border-b border-blue-300" id="tabs-menu"
          ...
          <li class="nav-item mr-1" role="presentation">
            <a href="#user-settings"
              ...
              aria-controls="user-settings" aria-selected="false">Disaster Management</a>
          </li>
        ...
        <div class="tab-content" id="tabs-user-options">
          ...
          >
            <user-profile v-bind:model="model" v-on:update="updateDisaster" :key="model.id+'profile'"/>
          </div> -->
          <div class="tab-pane fade show active" id="user-settings" role="tabpanel"
            aria-labelledby="tabs-user-settings">
            <user-settings v-bind:model="model" v-on:update="updateDisaster" :key="model.id + 'settings'" />
          </div>
...
import UserSettings from "../../../components/pages/disaster/settings.component.vue";
//SCHEMA//AND//STORES
...
const breadcrumbs = [
  ...
  { name: "Disaster Management", href: "/field/emergency-management", current: false },
  { name: "Manage", href: "#", current: true },
];

const DisasterStore = usecommodityinventoriestore();
...
const updateDisaster = async (newValues) => {
  isLoading.value = true;
  DisasterStore
    .update(newValues)
    .then((result) => {
    ...
    .catch((error) => {
      Swal.fire({
        title: "Failed",
        text: "failed to update disaster (" + error + ")",
        icon: "error",
        confirmButtonText: "Ok",
      });
    })
