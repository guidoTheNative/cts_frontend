      ...
      <div class="flex flex-wrap justify-center md:justify-start -mx-2 mt-7">
        <div v-for="option in options" :key="option.label" class="p-2 md:w-1/3 lg:w-1/5 relative group">
          ...
          <div v-if="option.label === 'Lean Season Response'" v-show="option.showDropdown"
            ...
            class="dropdown-menu">
            <router-link to="/warehouse/loadingplans"
              class="block px-4 py-2 text-gray-800 hover:bg-gray-200">
              Loading Plans
            </router-link>
            <router-link to="/warehouse/dispatches"
              class="block px-4 py-2 text-gray-800 hover:bg-gray-200">
              Dispatches
            </router-link>
          </div>
...
const options = ref([
  { label: 'Lean Season Response', icon: TemplateIcon, path: '/warehouse/dispatch-management', showDropdown: false },
  /* { label: 'Emergency Response', icon: LightningBoltIcon, path: '/activities' },
   */{ label: 'Donation Management', icon: HeartIcon, path: '/warehouse/donations' },
